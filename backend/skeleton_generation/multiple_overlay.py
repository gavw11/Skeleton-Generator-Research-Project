...
def process_frame(frame_index, frame, model):
    ...
    print(f"Finished processing in {time.monotonic() - start} seconds")    
...
def skeletonize(input_path, output_path, file_name):
    ...
    writer_process = Process(target=video_writer, args=(f"{output_path}\\{file_name}-skeleton.avi", result_queue, frame_count, width, height, fps))
